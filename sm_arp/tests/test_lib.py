...
def _chain_priors() -> dict[str, dict]:
...
def _verify_vector(v: dict, priors: dict[str, dict]):
...
def _action(category="data_shared", summary="did a thing", **kw):
...
def test_tamper_fails_at_signature():
    me = Identity.generate()
    r = issue_receipt(me, principal_did=me.did, action=_action())
...
def test_hash_chain_links_and_breaks():
    me = Identity.generate()
...
def test_authority_chain_scope():
...
def test_store_round_trip_and_idempotent(tmp_path):
...
@pytest.mark.parametrize("path", _VECTOR_PATHS, ids=lambda p: p.stem)
def test_verify_agrees_with_conformance(path):
