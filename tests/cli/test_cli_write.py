...
def test_keygen_with_seed_is_deterministic(runner):
    ...
    # Same seed → same DID
    did_a = next(
        line.split()[1] for line in strip_ansi(a.output).splitlines() if line.startswith("did:")
    )
    did_b = next(
        line.split()[1] for line in strip_ansi(b.output).splitlines() if line.startswith("did:")
    )
    assert did_a == did_b
...
def test_issue_message_sent_roundtrips(runner, tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            ...
            did,
            "--summary",
            "Test message from the write-side test suite.",
            "--out",
            str(out),
        ],
    )
...
def test_issue_short_seed_errors(runner):
    result = runner.invoke(
        ...
        [
            ...
            "short",
            "--principal",
            "did:key:z6MkABCDEFGHIJKLMNOPQRSTUVWXYZ12345",
            "--summary",
            "x",
        ],
    )
