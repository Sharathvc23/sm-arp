...
ARP is independent of the agent runtime (Claude, OpenClaw, OpenAI, framework-based, custom), the chapter or platform hosting the agent, and the underlying tool-integration or transport protocols (MCP, A2A). It is the human-facing accountability layer above those protocols.
...
8. Check that `principal_did` matches the principal binding expected for `issuer_did` (via a DAT grant or a chapter ledger record), and reject if mismatched.
...
An empty or absent `authority_chain` means the issuer relied on "standing authority" — the authority recorded at delegation time when the principal first bound their identity to the issuer. The audit trail for standing authority lives at the chapter or platform level, not inside individual receipts.
...
- **Member side** (signing, agency-log writes, push to chapter): `member-sdk/community_member/arp.py`.
- **Chapter side** (verification, hash-chain storage, retrieval API): `chapter/arp.py`, with HTTP endpoints in `chapter/chapter_agent.py` (`POST /api/receipts`, `GET /api/receipts`).
- **Conformance**: `conformance/arp/test_arp_v01.py`.

Other runtimes implementing ARP **MUST** pass `conformance/arp/`. The conformance suite is intentionally framework-agnostic; it loads vectors and verifies them against the JSON Schemas and Ed25519 verification — no NANDA-specific code paths are exercised.

## 16. Governance and stewardship
...
## 17. Open questions for v0.1 → v0.2
...
- W3C did:key Method — `spec/0.2/did-key.md` (NANDA Chapter Protocol reference) and the upstream W3C spec.
