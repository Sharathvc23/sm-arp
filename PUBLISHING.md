# Publishing sm-arp to PyPI

`sm-arp` publishes via **PyPI Trusted Publishing** — no API tokens. You tell PyPI
once that this repo's `release.yml` workflow may publish; after that, pushing a
version tag builds and uploads automatically.

> Do this only when `sm-arp` is meant to be **public**. (It is private today.)
...
2. Go to **https://pypi.org/manage/account/publishing/** → "Add a pending
   publisher" (use *pending*, because the `sm-arp` project doesn't exist on PyPI
   yet — the first publish creates it).
3. Fill the form with **exactly** these values:

   | Field | Value |
   |-------|-------|
   | PyPI Project Name | `sm-arp` |
   | Owner | `Sharathvc23` |
   | Repository name | `sm-arp` |
...
- The tag (`v0.2.0`) and `version` in `pyproject.toml` should match.
- A dry run anytime, no upload: `python -m build && python -m twine check dist/*`.
