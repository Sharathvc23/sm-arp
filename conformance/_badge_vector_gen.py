...
``vectors/arp/0.1/conformance/``. Vectors are deterministic: the
signing key seed and all timestamps are fixed, so re-running this
script produces byte-identical vector files.

The vectors exercise spec/arp/0.1/conformance.md §9 verification:
...
Outputs to ``vectors/arp/0.1/conformance/``.
"""
...
from conformance.badge import derive_did_key, sign_envelope
...
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "vectors" / "arp" / "0.1" / "conformance"
...
def _write(name: str, doc: dict[str, Any]) -> None:
    ...
    print(f"  wrote {out.relative_to(OUTPUT_DIR.parent.parent.parent.parent)}")
