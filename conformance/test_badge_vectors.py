...
``vectors/arp/0.1/conformance/`` per ``spec/arp/0.1/conformance.md`` §12.
...
``python -m conformance._badge_vector_gen`` produces byte-identical
...
from conformance.badge import VerificationError, sign_envelope, verify_envelope
from conformance.verify_badge import main as verify_badge_main

VECTORS_DIR = Path(__file__).resolve().parent.parent / "vectors" / "arp" / "0.1" / "conformance"
...
def test_valid_vector_is_byte_stable() -> None:
    ...
    """
    from conformance._badge_vector_gen import (
        FIXED_SIGNED_AT,
        RUNTIME_SEED,
        _base_payload,
    )
    ...
    assert regenerated == expected_envelope, (
        ...
        "Re-run `python -m conformance._badge_vector_gen` and commit the diff."
    )
